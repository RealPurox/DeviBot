...
public class AutoModEmojiHandler {
    ...
    public void handle(Command command, CommandSender sender) {
        String builder = DeviEmote.INFO.get() + " | " + devi.getTranslation(command.getLanguage(), 266) + " -> " + devi.getTranslation(command.getLanguage(), 273) + "\n\n" +
                "```python\n" +
    ...
    private void startWaiter(int attempt, Command command, CommandSender sender) {
        ...
        devi.getResponseWaiter().waitForResponse(event.getGuild(),
                ...
                response -> {
                    ...
                    if (entered == 1)
                        sender.reply(DeviEmote.SUCCESS.get() + " | " + devi.getTranslation(command.getLanguage(), 284));
                    else sender.reply(DeviEmote.SUCCESS.get() + " | " + devi.getTranslation(command.getLanguage(), 285));
                },
