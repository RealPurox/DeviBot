...
public class ModLogMessageDeleteHandler {
    ...
    public void handle(Command command, CommandSender sender) {
        String builder = DeviEmote.INFO.get() + " | " + devi.getTranslation(command.getLanguage(), 304) + " -> " + devi.getTranslation(command.getLanguage(), 336) + "\n\n" +
                "```python\n" +
    ...
    private void startWaiter(int attempt, Command command, CommandSender sender) {
        ...
        devi.getResponseWaiter().waitForResponse(event.getGuild(),
                ...
                response -> {
                    ...
                    if (entered == 1)
                        sender.reply(DeviEmote.SUCCESS.get() + " | " + devi.getTranslation(command.getLanguage(), 337));
                    else sender.reply(DeviEmote.SUCCESS.get() + " | " + devi.getTranslation(command.getLanguage(), 338));
                },
