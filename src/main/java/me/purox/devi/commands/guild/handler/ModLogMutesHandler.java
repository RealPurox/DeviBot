...
public class ModLogMutesHandler {
    ...
    public void handle(Command command, CommandSender sender) {
        String builder = DeviEmote.INFO.get() + " | " + devi.getTranslation(command.getLanguage(), 304) + " -> " + devi.getTranslation(command.getLanguage(), 342) + "\n\n" +
                "```python\n" +
    ...
    private void startWaiter(int attempt, Command command, CommandSender sender) {
        ...
        devi.getResponseWaiter().waitForResponse(event.getGuild(),
                ...
                response -> {
                    ...
                    command.getDeviGuild().getSettings().setBooleanValue(GuildSettings.Settings.MOD_LOG_MUTES, entered == 1);
