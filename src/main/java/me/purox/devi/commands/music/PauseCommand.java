...
public class PauseCommand implements Command {
    ...
    public void execute(String command, String[] args, MessageReceivedEvent event) {
        ...
        if (devi.getMusicManager().getPlayer(event.getGuild()).isPaused()) {
            MessageUtils.sendMessage(event.getChannel(), devi.getTranslation(language, 123));
            return;
        }

        devi.getMusicManager().getPlayer(event.getGuild()).setPaused(true);
        MessageUtils.sendMessage(event.getChannel(), devi.getTranslation(language, 124, "`" + prefix + "resume`"));
    }
